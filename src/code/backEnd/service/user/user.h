...
namespace dmc {
	...
	class User {
	public:
		User(const cjson::Json& _userData, http::Server* _serviceToListen);
	...
	private:
		...
		http::Response		addDevice		(const cjson::Json& _deviceData);
		...
		cjson::Json			deviceListJson	() const;
		void				updateDevices	();
		void				loadDevices		(const cjson::Json& _deviceList);
